    ...
    <Cutout className="bg-[#c6c6c6] w-full h-full p-1">
      <div className="border-2 border-white border-b-gray-800 border-r-gray-800 p-4 sm:p-6 flex flex-col h-full bg-[#c6c6c6]">
        ...
        <div className="flex items-center gap-4 mb-2">
          {/* Classic icon placeholder using a folder/file cabinet metaphor */}
        ...
        <div className="flex-1 overflow-y-auto pr-2 max-h-[50vh] space-y-4 scrollbar-win95">
          {entries.length === 0 ? (
            <p className="text-sm italic text-black">
              No active or historical processes found. Check back later.
            </p>
          ) : (
            entries.map((entry) => (
              <GroupBox 
                key={entry.id} 
                label={entry.period || "Process Runtime"} 
                className="text-black bg-[#c6c6c6]"
              >
                <div className="p-2 space-y-3">
                  {/* Title and Subtitle (Company Name) */}
                  <div>
                  ...
                  {/* Sunken text box for the job responsibilities */}
                  ...
                  {/* 3D raised tags for technologies and skills used */}
